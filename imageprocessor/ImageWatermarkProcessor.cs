...
namespace ImageProcessor
{
    public class ImageWatermarkProcessor
    {
        ...
        public ImageWatermarkProcessor(ILogger<ImageWatermarkProcessor> logger, IConfiguration configuration)
        {
            _logger = logger;
        ...
        [Function(nameof(ImageWatermarkProcessor))]
        [ServiceBusOutput("archive-queue", Connection = "ServiceBusConnection")]
        public async Task<string> Run(
            [ServiceBusTrigger("watermark-queue", Connection = "ServiceBusConnection")] ServiceBusReceivedMessage message)
        {
            _logger.LogInformation("Received message: {MessageId} from watermark-queue", message.MessageId);
            try
            {
            ...
            catch (Exception ex)
            {
                ...
                await _serviceBusClient.CreateReceiver("watermark-queue").CompleteMessageAsync(message);
                throw;
            }
        }
    }
